...
/**
 * This class pulls down a docker image of a open telemetry smoke test app and runs it with the
 * bundle attached
 */
public class Bundle {
  ...
  private static final String DOCKER_IMAGE_NAME =
      "open-telemetry-docker-dev.bintray.io/java/smoke-springboot-jdk11:latest";
  ...
  public void start() {
    ...
    String args =
        "-javaagent:/newrelic-opentelemetry-javaagent.jar "
            ...
            + String.format(
                "-Dnewrelic.trace.uri.override=http://%s:8080/trace/add ", MockEdge.NETWORK_ALIAS)
            + String.format(
                "-Dnewrelic.metric.uri.override=http://%s:8080/metric/add ", MockEdge.NETWORK_ALIAS)
    ...
    target =
        new GenericContainer<>(DockerImageName.parse(DOCKER_IMAGE_NAME))
            ...
            .withEnv("JAVA_TOOL_OPTIONS", args)
